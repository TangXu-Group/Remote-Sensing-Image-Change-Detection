...
#         print(output.shape)
#         output = torch.argmax(output.detach(), dim=0)
...
#         output = torch.unsqueeze(output, dim=0)
    ...
    @staticmethod
    def get_target_width_height(input1_tensor):
        width, height = input1_tensor.size(-1), input1_tensor.size(-2) #这种写法很有趣，可以记录一下
        return width, height
    ...
    def compute_cam_per_layer(self, input1_tensor):
        ...
        target_size = self.get_target_width_height(input1_tensor)
    ...
    def __call__(self, input1_tensor, input2_tensor, target): #创建该类后自动调用__call__()方法
        ...
        # 一定要注释掉！！！
        output = self.activations_and_grads(input1_tensor, input2_tensor)[0]
        print(output.shape)
        ...
        output1 = torch.split(output,1,dim=0)
        output = output1[0]
        loss = self.get_loss(output, target)
        loss.backward(torch.ones_like(target), retain_graph=True)

        cam_per_layer = self.compute_cam_per_layer(input1_tensor) #得到每一层的CAM，返回的是一个列表
